...
# Import libraries--------------------------------------------------------------
pacman::p_load(tidyverse, truncnorm, sloop, ozmaps, sf, patchwork, ggmagnify, metR)
...
count_ToE_hist_v2 <- specific_CO2_comp_custom_bins_tim_of_emergence |> 
  summarise(
    n = n(),
    .by = custom_bins
  ) |> 
...
# Maybe have two maps one for zooming and one for plotting
...
# TODO:
# - Apply same formatting to plot below (colours to binned, titles etc)
# - Apply dots to inset_plot_STATE
# - Move legend inside of plot -> left of NSW inset

# Give each magnify its own aes
# Slopes geom_magnify double plotting and doubles lines
# These must have the same points
...
inset_plot_VIC <- aus_map |> 
  ...
  geom_point(
    data = vic_data,
    aes(x = lon, y = lat, colour = binned_evidence_ratio, shape = impact_of_CO2_term),
    show.legend = FALSE
  ) +
  theme_void() 
...
# Magnify sections:
# - Easiest is use squares
# - I can write custom shapes (later)
#shape = "outline" # outline requires exact scale because sf cannot scale  ) +
...
single_map_aus <- aus_map |> 
  ...
  geom_point(
    ...
    mapping = aes(x = lon, y = lat, colour = binned_evidence_ratio, shape = impact_of_CO2_term)
  ) + 
  theme_bw() +
  # expand map
  coord_sf(xlim = c(105, 180), ylim = c(-60, -10)) +
  # magnify WA
  geom_magnify(
    from = c(114, 120, -36, -30),
    to = c(106, 116, -20, -10),
    shadow = FALSE,
    expand = 0,
    plot = inset_plot_WA,
