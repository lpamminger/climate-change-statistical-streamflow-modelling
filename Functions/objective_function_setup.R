...
objective_function_setup_v2 <- function(streamflow_model, transform_parameter_methods, lower_bound, upper_bound, objective_function, catchment_data, scale, minimise_likelihood) {
  force(catchment_data)
  force(streamflow_model)
  force(objective_function)
  force(transform_parameter_methods)
  ...
  function(parameter_set) {
    ...
    LL_scores <- purrr::pmap(
      .l = list(
        modelled_streamflow,
        observed_streamflow # ,
        # stop_start_data_set - this is a by-product of CO2 obj function not in constant
      ),
      .f = objective_function,
      parameter_set = real_parameter_set
    )
