...
convert_to_dream_form <- function(DREAM_controls) {
  ...
  if (DREAM_controls$thinning == 1) {
    DREAM_controls$thinning <- NA
  }
  ...
  list(
    ...
    parallel = DREAM_controls$parallel
  )
}
...
make_DREAM_bounds_and_transform_methods <- function(all_gauges_best_CMAES_parameters) {
  # Function factory - given CMAES parameters before making bounds
  force(all_gauges_best_CMAES_parameters)

  function(catchment_data_set) {
    ...
    ## Default tibble ==========================================================
    bound_and_transforms <- tibble::tribble(
    ...
    # The CO2 parameter bounds require reduced bounds for DREAM ------------------
    gauge <- catchment_data_set$gauge

    gauge_best_parameters <- all_gauges_best_CMAES_parameters |>
      filter(gauge == {{ gauge }})

    gauge_parameter_names <- gauge_best_parameters |>
      pull(parameter)


    # case_when will call the function after `~` even if the condition is not meet
    # use if statements with slope/intercept to call function
    type_of_CO2_impact <- case_when(
      any(gauge_parameter_names %in% c("a3_intercept")) ~ "intercept",
      any(gauge_parameter_names %in% c("a3_slope")) ~ "slope",
      .default = "no_CO2_component"
    )




    if (type_of_CO2_impact == "intercept") {
      all_a3_intercept_values <- all_gauges_best_CMAES_parameters |>
        filter(parameter == "a3_intercept") |>
        pull(parameter_value)

      calibrated_intercept_value <- gauge_best_parameters |>
        filter(parameter == "a3_intercept") |>
        pull(parameter_value)

      altered_a3_intercept_bounds <- make_DREAM_a3_bounds(
        catchment_a3_value = calibrated_intercept_value,
        all_catchments_a3_values = all_a3_intercept_values
      )

      bound_and_transforms <- bound_and_transforms |>
        rows_update(
          tibble(
            parameter = "a3_intercept",
            lower_bound = altered_a3_intercept_bounds[1],
            upper_bound = altered_a3_intercept_bounds[2]
          ),
          by = "parameter"
        )
    } else if (type_of_CO2_impact == "slope") {
      all_a3_slope_values <- all_gauges_best_CMAES_parameters |>
        filter(parameter == "a3_slope") |>
        pull(parameter_value)

      calibrated_slope_value <- gauge_best_parameters |>
        filter(parameter == "a3_slope") |>
        pull(parameter_value)

      altered_a3_slope_bounds <- make_DREAM_a3_bounds(
        catchment_a3_value = calibrated_slope_value,
        all_catchments_a3_values = all_a3_slope_values
      )
      ...
      bound_and_transforms <- bound_and_transforms |>
        rows_update(
          tibble(
            parameter = "a3_slope",
            lower_bound = altered_a3_slope_bounds[1],
            upper_bound = altered_a3_slope_bounds[2]
          ),
          by = "parameter"
        )
    }

    return(bound_and_transforms)
  }
}
...
DREAM <- function(input, controls) {
  ...
  dream_result$real_coefs <- pmap(
    .l = list(
      dream_result$numerical_optimiser_setup$parameter_transform_method,
  ...
  transform_to_realspace_dream(
    ...
    transform_functions = numerical_optimiser_setup$parameter_transform_method,
