...
warm_to_cool_season_rainfall_ratio <- function(monthly_rainfall, month, cool_seasons, warm_seasons) {
  ...
  if(any(sort(month) != sort(c(cool_seasons, warm_seasons)))) {message("Missing months")}
...
yearly_streamflow <- daily_streamflow_raw |> 
                       ...
                       mutate(
                         q_mm = if_else(q_mm < 1, 1, q_mm) #q_mm + .Machine$double.eps^0.5 # some streamflow values are less than one. When bc'd this produces a negative boxcox streamflow value = bad 
                       ) |> 
                       arrange(gauge, year)  
...
min_run_length <- 2 # I can change this to what every I want 
...
list_start_end_index <- map(.x = unique(yearly_data$gauge),
                            .f = gauge_continous_start_end,
                            data = yearly_data,
                            min_run_length = min_run_length) # minimum of 2 years of data
...
gauge_data <- yearly_data |> 
                summarise(
                  bc_lambda = boxcox_lambda_generator(precipitation = p_mm, streamflow = q_mm),
...
list_continuous_yearly_data <- pmap(.l = list(
...
bc_q_generator <- function(gauge_id, a_priori_boxcox_lambda, yearly_data) {
  
  extracted_q_mm <- yearly_data |> 
                      filter(gauge == gauge_id) |> 
                      dplyr::select(q_mm)
  
  if(any(extracted_q_mm[!is.na(extracted_q_mm)] <= 0)){stop("Cannot transform value less than zero")}
  
  boxcox_transform(extracted_q_mm, lambda = a_priori_boxcox_lambda)
}


bc_q_list <- map2(.x = gauge_data$gauge,
...
with_NA_bc_q_list <- map2(.x = gauge_data$gauge,
                          .y = gauge_data$bc_lambda,
                          .f = bc_q_generator,
